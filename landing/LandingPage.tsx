...
function LoadingOverlay() {
  ...
  useEffect(() => {
    ...
    function dismiss() {
      ...
      setTimeout(() => {
        setExit(true);
        // clip-path transition takes 1.4s, then unmount + trigger page drop-in
    ...
    <div id="s5-loader" className={exit ? 's5-loader--exit' : ''}>
      <div id="s5-loader-content">
        <div id="s5-loader-seq">
          {/* sonar — concentric circles + crosshair */}
          ...
          {/* shoot — targeting with corner brackets and tick marks */}
...
// Pre-generated dot cloud for Predictive Maintenance card
const DOT_POSITIONS = (() => {
...
function SolSvg({ idx }: { idx: number; active: boolean }) {
  if (idx === 0) return (
    <svg viewBox="0 0 480 360" fill="none" stroke="currentColor" strokeWidth="1" className="s5-sol-icon">
      ...
      <ellipse cx="240" cy="180" rx="89" ry="112"/>
      <ellipse cx="240" cy="180" rx="59" ry="112"/>
      <ellipse cx="240" cy="180" rx="26" ry="112"/>
      <ellipse cx="240" cy="148" rx="107" ry="21"/>
      <ellipse cx="240" cy="212" rx="107" ry="21"/>
      <ellipse cx="240" cy="116" rx="84" ry="14"/>
      <ellipse cx="240" cy="244" rx="84" ry="14"/>
      <ellipse cx="240" cy="91"  rx="48" ry="7"/>
      <ellipse cx="240" cy="269" rx="48" ry="7"/>
      <line x1="240" y1="68" x2="240" y2="292"/>
      <line x1="128" y1="180" x2="352" y2="180"/>
...
export function LandingPage() {
  ...
  const [binaryRows, setBinaryRows] = useState(() => Array.from({ length: 9 }, genBinaryBlock));
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      const newH = 0.25 + Math.random() * 0.65;
      setGraphH(newH);
  ...
  // Scroll-sync: drive right grid scrollTop from page scroll while left card is stuck
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (top <= 0) {
        ...
        grid.scrollTop = Math.min(-top, maxScroll);
      } else {
        grid.scrollTop = 0;
      }
    };
  ...
  async function handleConnect() {
    if (status === "connected") { navigate("/trade"); return; }
    await connect(); navigate("/trade");
  }

  // Infinite carousel — clone CAROUSEL_CLONES items at each end
  const allCards = [
  ...
  const handleCarouselTransitionEnd = useCallback(() => {
    ...
    if (cardIdx >= SOLUTIONS.length + CAROUSEL_CLONES) {
      setCardIdx(CAROUSEL_CLONES);
    } else if (cardIdx < CAROUSEL_CLONES) {
      setCardIdx(SOLUTIONS.length + CAROUSEL_CLONES - 1);
    }
  }, [cardIdx]);
  ...
  const graphPolyline = graphXs.map((x, i) => `${x},${graphPoints[i] ?? 20}`).join(" ");
  ...
  return (
    <>
      ...
      {createPortal(
        ...
        >
          <img src="/images/kryon.png" alt="Kryon" className="s5-nav-logo-img" />
    ...
    <div id="s5-page-root" className="s5-page">
        ...
        {menuOpen && (
          <div className="s5-menu-overlay" onClick={() => setMenuOpen(false)}>
            <div className="s5-menu-panel" onClick={e => e.stopPropagation()}>
              ...
              {LANDING_NAV.map(n => (
                <Link key={n.to} to={n.to} className="s5-menu-nav-link" onClick={() => setMenuOpen(false)}>
                  {n.label}
                </Link>
              ))}
      ...
      {/* ════ MAIN GRID — left scrolls, right sticks ════ */}
      <div className="s5-main-grid">
        ...
        {/* ── RIGHT COLUMN — CSS sticky ── */}
        <div className="s5-right-col">

          {/* Gray hero-right card */}
          ...
          <Link to="/trade" className="s5-right-cta">
      ...
      <section className="s5-solutions">
        ...
        <div
          className="s5-solutions-viewport"
          ...
          <div className="s5-insights-featured-art">
            <div className="s5-insights-featured-art-img">
              <svg viewBox="0 0 800 400" fill="none" stroke="rgba(181,153,229,0.75)" strokeWidth="1">
                ...
                <ellipse cx="220" cy="310" rx="55" ry="55"/>
                <ellipse cx="220" cy="310" rx="30" ry="30"/>
                <ellipse cx="580" cy="310" rx="55" ry="55"/>
                <ellipse cx="580" cy="310" rx="30" ry="30"/>
      ...
      {/* ════ CTA ════ */}
      {/* <section className="s5-cta s5-reveal" ref={ctaRef}>
        <h2 className="s5-cta-title">
          Ready to harness your<br />
          Op<span className="s5-cta-slash">/</span>Intelligence?
        </h2>
        <div className="s5-cta-input-wrap">
          <span className="s5-cta-bracket">(</span>
          <input type="email" placeholder="Enter your email" className="s5-cta-input" />
          <span className="s5-cta-bracket">)</span>
        </div>
        <button className="s5-continue-btn">Continue</button>
      </section> */}
      ...
      <footer className="s5-footer">
        ...
        <div className="s5-footer-signup">
          ...
          <button
            className="s5-footer-subscribe-btn"
            onClick={handleFtSubscribe}
            disabled={ftSubscribed}
          >
