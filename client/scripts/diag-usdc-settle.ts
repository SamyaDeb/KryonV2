...
async function main() {
  ...
  const tx = new TransactionBuilder(account, { fee: FEE, networkPassphrase: NETWORK.passphrase })
    .addOperation(new Contract(CONTRACTS.orderGateway).call("settle_fill", fillArg))
    .setTimeout(60).build();
  const sim = await server.simulateTransaction(tx);
  if ((rpc.Api as any).isSimulationError(sim)) {
    console.log("\nFULL ERROR:", (sim as any).error);
  } else {
    console.log("✓ SUCCESS");
  }
  process.exit(0);
