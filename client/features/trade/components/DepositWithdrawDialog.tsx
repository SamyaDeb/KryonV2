...
export function DepositWithdrawDialog({
  ...
  triggerClassName = "h-7 text-xs rounded-[6px] border border-[#34343a] bg-[#26262b] hover:border-[#3f3f47] text-[#e6e6e6] px-3 transition-colors",
  defaultTab = "deposit",
}: {
  triggerLabel?: string;
  ...
  const pill = (active: boolean) =>
    `rounded-[8px] py-2 text-[13px] font-semibold transition-colors ${
      active ? "bg-[#26262b] text-[#e6e6e6]" : "text-[#8a8f97] hover:text-[#e6e6e6]"
    }`;
      ...
      {mounted && open &&
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            />
            <div className="relative w-[380px] max-w-full rounded-xl border border-[#34343a] bg-[#141416] text-[#e6e6e6] shadow-[0_20px_60px_rgba(0,0,0,.6)]">
              <div className="p-5">
                {/* Header */}
                <div className="flex items-center justify-between">
                  <h2 className="text-[17px] font-bold text-[#e6e6e6]">Collateral</h2>
                  <button
                    onClick={() => setOpen(false)}
                    aria-label="Close"
                    className="w-7 h-7 grid place-items-center rounded-[6px] text-[#8a8f97] hover:text-[#e6e6e6] hover:bg-[#26262b] transition-colors"
                  >
                    <X size={16} />
                  </button>
                </div>
                ...
                {/* Tabs */}
                <div className="mt-4 grid grid-cols-2 gap-1 rounded-[10px] border border-[#34343a] bg-[#1b1b1f] p-1">
                ...
                {/* Amount panel */}
                <div className="mt-4 rounded-[12px] border border-[#34343a] bg-[#26262b] p-4">
                  <div className="mb-2 flex items-center justify-between text-[12px] text-[#8a8f97]">
                    <span>Amount</span>
                    {tab === "withdraw" && (
                      <div className="flex items-center gap-2 text-[11.5px]">
                        <button
                          onClick={() => setAmount((balanceHuman / 2).toFixed(2))}
                          className="rounded-full bg-[#1b1b1f] border border-[#34343a] px-2.5 py-0.5 font-semibold text-[#8a8f97] hover:text-[#e6e6e6] transition-colors"
                        >
                          50%
                        </button>
                        <button
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <input
                      ...
                      className="flex-1 min-w-0 bg-transparent outline-none border-0 text-[26px] font-semibold text-[#e6e6e6] tabular"
                    />
                    <span className="flex shrink-0 items-center gap-1.5 text-[15px] font-semibold text-[#e6e6e6]">
                ...
                <div className="mt-3 flex items-center justify-between px-1 text-[12.5px]">
                  ...
                  <span className="flex items-center gap-1.5 tabular text-[#e6e6e6]">
                    <UsdcLogo size={12} />
                ...
                <button
                  ...
                  className="mt-5 w-full h-12 rounded-[10px] text-[14px] font-bold text-[#1a1205] bg-[#f7931a] hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed transition"
                >
                  {loading ? "Confirming…" : overMax ? "Insufficient balance" : tab === "deposit" ? "Deposit USDC" : "Withdraw USDC"}
                </button>

                <p className="mt-3 text-[11px] text-[#5a5f67] text-center">
